...
function renderHome() {
    ...
    if (newsContainer && typeof newsData !== 'undefined') {
        ...
        const latestNews = sorted.slice(0, 5); // 5개만 표시
...
function renderPublications() {
    ...
    if (startInput && endInput && publicationData.length > 0) {
        ...
        startInput.min = Math.min(...years);
        startInput.max = Math.max(...years);
        endInput.min = Math.min(...years);
        endInput.max = Math.max(...years);
        startInput.placeholder = Math.min(...years);
        endInput.placeholder = Math.max(...years);
    }

    updateVenueOptions('all');
    applyPubFilter(); // 초기 로드
    ...
    buttons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            updateVenueOptions(btn.dataset.cat);
            applyPubFilter();
        });
    });
}
...
function renderPubPage(page) {
    ...
    batch.forEach(pub => {
        ...
        if (videoUrl) {
            linkButtons += `
                <a href="${videoUrl}" class="pub-link btn-video" target="_blank">
...
function renderPagination() {
    ...
    if (currentPubList.length <= itemsPerPage) return; // 페이지 1개면 버튼 안 만듦
    ...
    container.parentNode.appendChild(pagDiv); // pub-list 바로 뒤에 추가
}
